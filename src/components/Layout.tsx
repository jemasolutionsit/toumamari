...
export function Layout({ children, scrolled }: { children: ReactNode; scrolled: boolean }) {
  ...
  return (
    <div className="min-h-dvh bg-neutral-50 font-sans text-neutral-900 selection:bg-[#FFD700] selection:text-black flex flex-col">
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center pt-safe">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <button type="button" onClick={() => setIsCartOpen(true)} aria-label={t.cart} className="relative group text-white hover:text-[#FFD700] transition-colors flex items-center gap-2">
        ...
        {/* Mobile menu panel — absoluto bajo la barra y animado solo con
            transform/opacity (animar height provoca reflow en cada frame) */}
        <AnimatePresence>
          {menuOpen && (
            <motion.nav
              ...
              className="lg:hidden absolute top-full left-0 right-0 -mt-px bg-black/95 backdrop-blur-md border-t border-white/10 shadow-xl"
            >
      ...
      {/* FOOTER */}
      <footer className="bg-neutral-950 text-neutral-400 py-16 pb-[calc(4rem+env(safe-area-inset-bottom))] mt-auto">
